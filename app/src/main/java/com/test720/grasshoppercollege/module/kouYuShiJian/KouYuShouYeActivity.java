...
import com.test720.grasshoppercollege.HttpBean.SpeekingTimeListData;
...
public class KouYuShouYeActivity extends BaseShouQuanXianActivity {
    ...
    List<SpeekingTimeListData.DataBean> list = new ArrayList<>();
    ...
    protected void initData() {
        adapter = new MyBaseRecyclerAdapter<SpeekingTimeListData.DataBean>(list, this, R.layout.speek_time_item) {
            @Override
            public void convert(BaseRecyclerHolder holder, SpeekingTimeListData.DataBean item, int postion) {
        ...
        adapter.setOnItemClickListener(new MyBaseRecyclerAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                ...
                re_id = list.get(position).getOral_id();
                ...
                if (canSee) {
                    Intent in = new Intent(mcontext, SpeekTimeVideoActivity.class);
                    in.putExtra("oral_id", list.get(position).getOral_id());
    ...
    @Override
    public String url() {
        return HttpUntil.GetIntent().EnOralindex();
    }

    @Override
    public void shuaXin(String str) {
        if (codeIsOne(str, false)) {
            SpeekingTimeListData data = new Gson().fromJson(str, SpeekingTimeListData.class);
            list.clear();
