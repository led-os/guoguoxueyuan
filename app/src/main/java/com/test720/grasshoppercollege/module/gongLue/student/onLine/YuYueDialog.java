 ...
 * 作者：水东流 编于 2018/9/19
 */
public class YuYueDialog extends BaseOkDialogFragment {
    @BindView(R.id.name)
    EditText name;
    @BindView(R.id.phone)
    EditText phone;
    ...
    @Override
    public int layoutId() {
        return R.layout.yu_yue_course_dialog;
    }
        ...
        switch (view.getId()) {
            case R.id.ok:
                ...
                if (TextUtils.isEmpty(phone.getText())) {
                    phone.setError("请留下您的联系方式");
                    return;
                }
                if (phone.getText().toString().length() < 11) {
                    phone.setError("请留下正确的11位联系方式");
                    return;
                }
                if (TextUtils.isEmpty(nianji.getText())) {
                    nianji.setError("请选择年级");
                    return;
                }
                if (click != null) {
                    click.nameAndPhone(name.getText().toString(), phone.getText().toString(), nianji.getText().toString());
    ...
    public interface Click {
        void nameAndPhone(String name, String phone, String nianji);
    }
}
